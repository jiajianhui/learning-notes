# 16. 登录和安全：服务器怎样确认管理员身份

> Mini CMS 阶段 6：先完成标签、发布规则和管理页面，再用本章理解登录闭环；具体代码放到第 16A 章实践。
...
下一步不要停在概念层。进入 [16A-管理员登录实操](./16A-管理员登录实操-用Session和Cookie保护写接口.md)，亲手完成密码哈希、数据库 Session、HttpOnly Cookie 和写接口保护。
