...
阶段 5：回看 06 API 设计、08 筛选分页，阅读 15 多表关系和事务，用 15A 复习关系维护，按 16 完成页面联调，再用 16A 梳理数据流，按 16B 完善普通 Ant Design 的 UI、16C 用 ProComponents 简化后台
...
第 14 章先从头到尾完成 Ant Design 管理后台，不在跟练中途切换章节。完成页面和联调后，先阅读 [14A-从页面操作到数据库：一条线看懂管理后台 CRUD](./14A-从页面操作到数据库-一条线看懂管理后台CRUD.md)，从页面操作一路追到 PostgreSQL，并沿成功数据或错误返回页面；再用 [14B-管理后台里的 TypeScript、Promise 和 React 状态](./14B-管理后台里的TypeScript-Promise和React状态.md) 集中复习泛型、Swift 与 TypeScript 的类型差异、异步返回和状态快照。

完成第 15 章后，阅读 [15A-从单表 CRUD 到关系维护](./15A-从单表CRUD到关系维护.md)，串起 Article、Tag、ArticleTag 在创建、更新和返回数据时分别承担什么。随后按 [16-标签管理与页面联调](./16-标签管理与页面联调.md) 完成标签接口、管理页面和种子数据，再用 [16A-从表单到数据库再回到页面](./16A-从表单到数据库再回到页面.md) 梳理值、类型和字段在前后端之间的变化。第 15、16 章完成阶段 5 的业务链路；15A、16A 分别是两章的配套巩固，不增加新的实操任务。

读完 16A，按 [16B-用 Ant Design 完善后台 UI](./16B-用AntDesign完善后台UI.md) 理解布局、主题、组件组合与状态反馈，自主还原目标页面并记录关键选择。再按 [16C-用 ProComponents 简化管理后台](./16C-用ProComponents简化管理后台.md) 在同一个 `admin-web-antd` 中改造列表、表单和布局，保持相同的 UI 目标，完成后回到[第 10 章](./10-MiniCMS项目总览.md)验收阶段 5。普通组件版通过 Git 保留，后续登录、测试和部署沿用改造后的工程，不新增第三套后台。

两章新增的动手收口：

- [17A-管理员登录实操](./17A-管理员登录实操-用Session和Cookie保护写接口.md)：把密码哈希、数据库 Session、HttpOnly Cookie 和认证中间件连起来。
- [18A-接口测试实操](./18A-接口测试实操-用Vitest和Supertest验证API.md)：用独立测试数据库、Vitest 和 Supertest 验证核心 API。
...
第 19、19A 章对应 Mini CMS 必做阶段 8；第 20～23 章用于阅读、排错和复习，不要求第一次连续背完。

### 第六部分：完成 shadcn/ui 并列后台项目

```text
24 shadcn/ui 为什么不是传统组件库
25 用 Axios 建立后台骨架并复用登录 API
26 用 TanStack Table 完成文章列表
26A 从数据到表格实例
27 用 React Hook Form 和 Zod 完成文章表单
27A 表单状态和两次 Zod 校验
28 独立完成标签管理并比较两套后台
```
...
第 10 章负责衔接阶段性 CRUD 实操与 Mini CMS，并维护项目范围、实现顺序和完成标准。第 11～19 章负责讲清当前知识，并把它落到 Mini CMS 中。
...
第一次阅读只跟正常流程，不要求提前记住大量故障和工程误区。开始写代码后，再使用第 21 章排错，并对照第 10 章每个阶段的验收标准。
...
读完第 09 章后，先用 09A 收口 Prisma 主线，再读第 10 章完成从 demo 到 Mini CMS 的过渡。阶段 3 先补上请求校验、统一错误处理和项目结构；阶段 4 用第 13 章完成前后端过渡，完整学完第 14 章并完成 Ant Design 后台后，再用 14A 建立完整 CRUD、数据返回和错误传播主线，用 14B 收口 TypeScript、Promise 和 React 状态机制；阶段 5～7 继续增加标签、登录和测试。共享 API 稳定后，再按第 24～28 章完成 shadcn/ui 后台；最后完成阶段 8 的公开 API、个人网站文章详情页和真实部署。两套后台和个人网站公开阅读链路都是学习目标。
