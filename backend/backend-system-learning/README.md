...
阶段 4：阅读 13 前后端衔接和 fetch，再用 14 完成 admin-web-antd 跟练，并用 14A 串起数据和错误传递
...
第 14 章读到第 12.3 节时，用 [14A-数据和错误怎样在页面与表单之间传递](./14A-数据和错误怎样在页面与表单之间传递.md) 串起 `ArticlesPage`、`ArticleForm` 和 API 请求之间的成功数据、错误和状态，再回到第 14 章继续跟练。
