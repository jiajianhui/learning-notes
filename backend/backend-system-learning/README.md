...
阶段 3：阅读 11 请求校验、12 项目结构
阶段 4：阅读 13 前后端联调
阶段 5：阅读 14 多表关系、JOIN 和事务
阶段 6：阅读 15 登录与安全、15A 登录实操
阶段 7：阅读 16 测试分层、16A 接口测试实操
阶段 8：阅读 17 运行和部署
```
...
- [15A-管理员登录实操](./15A-管理员登录实操-用Session和Cookie保护写接口.md)：把密码哈希、数据库 Session、HttpOnly Cookie 和认证中间件连起来。
- [16A-接口测试实操](./16A-接口测试实操-用Vitest和Supertest验证API.md)：用独立测试数据库、Vitest 和 Supertest 验证核心 API。
...
第 10 章负责衔接阶段性 CRUD 实操与 Mini CMS，并维护项目范围、实现顺序和完成标准。第 11～17 章负责讲清当前知识，并把它落到 Mini CMS 中。
...
第一次阅读只跟正常流程，不要求提前记住大量故障和工程误区。开始写代码后，再使用第 19 章排错，并对照第 10 章每个阶段的验收标准。
...
读完第 09 章后，先用 09A 收口 Prisma 主线，再读第 10 章完成从 demo 到 Mini CMS 的过渡。阶段 3 先补上请求校验、统一错误处理和项目结构，阶段 4 再完成管理页面，后面逐步增加标签、登录、测试和部署。阶段 5 增加文章标签时，才要求掌握中间表、`JOIN` 和事务。
