...
阶段 5：回看 06 API 设计、08 筛选分页，阅读 15 多表关系和事务，用 15A 复习关系维护，按 16 完成页面联调，再用 16A 梳理数据流、16B 理解组件拆分，按 16C 完善普通 Ant Design 的 UI、16D 用 ProComponents 简化后台
...
随后按 [16C-用 Ant Design 完善后台 UI](./16C-用AntDesign完善后台UI.md) 理解布局、主题、组件组合与状态反馈，自主还原目标页面并记录关键选择。再按 [16D-用 ProComponents 简化管理后台](./16D-用ProComponents简化管理后台.md) 在同一个 `admin-web-antd` 中改造列表、表单和布局，保持相同的 UI 目标。普通组件版通过 Git 保留，后续登录、测试和部署沿用改造后的工程；不新增第三套后台，也不要求先实践 16B 的全部拆分方案。
