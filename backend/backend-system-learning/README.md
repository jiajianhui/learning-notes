...
概念章节使用文章接口作为统一示例，但知识结构不依赖某个具体项目。最终实践项目单独放在第 21 章。
...
读完第 09 章就可以打开第 21 章开始实操，不需要先读完后面所有章节。先完成 Mini CMS 的阶段 1～2；准备进入阶段 3 时，再学习第 10 章的请求校验和统一错误处理。
...
阶段 3：阅读 10 请求校验、12 项目结构
阶段 4：阅读 13 前后端联调
阶段 5：阅读 11 多表关系、JOIN 和事务
...
第 16 章在准备部署时看；第 17～20 章用于阅读、排错和复习，不要求第一次连续背完。
...
前面的概念章节负责建立通用知识，第 21 章只负责安排项目范围、实现顺序和完成标准。
...
第一次阅读只跟正常流程，不要求提前记住大量故障和工程误区。开始写代码后，再使用第 18 章排错，并对照第 21 章每个阶段的“实现时检查”。
...
读完第 09 章后，按第 21 章开始 Mini CMS。阶段 3 先补上请求校验、统一错误处理和项目结构，阶段 4 再完成管理页面，后面逐步增加标签、登录、测试和部署。阶段 5 增加文章标签时，才要求掌握中间表、`JOIN` 和事务。
