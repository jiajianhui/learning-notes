...
阶段 4：阅读 13 前后端衔接和 fetch，用 14 完成 admin-web-antd 跟练，再用 14A、14B、14C 依次复习完整 CRUD、页面内部传递和语言机制
...
第 14 章先从头到尾完成 Ant Design 管理后台，不在跟练中途切换章节。完成页面和联调后，先阅读 [14A-从页面操作到数据库：一条线看懂管理后台 CRUD](./14A-从页面操作到数据库-一条线看懂管理后台CRUD.md)，从页面操作一路追到 PostgreSQL，并沿成功数据或错误返回页面；再阅读 [14B-数据和错误怎样在页面与表单之间传递](./14B-数据和错误怎样在页面与表单之间传递.md)，放大 `ArticlesPage`、`ArticleForm` 和 API 请求之间的回调、Promise、状态与错误；最后用 [14C-管理后台里的 TypeScript、Promise 和 React 状态](./14C-管理后台里的TypeScript-Promise和React状态.md) 集中复习泛型、Swift 与 TypeScript 的类型差异、异步返回和状态快照。
...
读完第 09 章后，先用 09A 收口 Prisma 主线，再读第 10 章完成从 demo 到 Mini CMS 的过渡。阶段 3 先补上请求校验、统一错误处理和项目结构；阶段 4 用第 13 章完成前后端过渡，完整学完第 14 章并完成 Ant Design 后台后，再用 14A 建立完整 CRUD 主线，用 14B 深入页面内部的数据与错误传递，用 14C 收口 TypeScript、Promise 和 React 状态机制；阶段 5～7 继续增加标签、登录和测试。共享 API 稳定后，再按第 23～27 章完成 shadcn/ui 后台；阶段 8 的部署仍然可选。两套前端都是学习目标，只是按顺序实现，避免同时学习两套组件体系。
