...
前面的章节负责建立后端技术地图，这一章负责把它们连成一个可以操作的项目：
...
这不是新的知识章节，而是项目的任务和进度清单。读完 00～09 章就可以开始；后面遇到新任务时，再按各阶段的链接回看对应章节。`13A` 始终是选读。
...
| 3 | 文章 CRUD 和管理页面闭环 | 完成文章 CRUD 接口，以及列表、新建、编辑和删除操作 |
| 4 | 标签、筛选、分页和发布规则可用 | 建立标签关系，完成标签管理、筛选分页、发布撤回和事务更新 |
| 5 | 管理员登录和写接口保护完成 | 建立管理员和 Session 数据，用 Cookie 与认证中间件保护管理接口 |
| 6 | 核心 API 可自动验证，项目可复现 | 增加自动化测试、README 和项目复盘 |
| 7（可选） | 个人网站可以读取已发布内容 | 提供已发布文章 API，接入个人网站并完成部署 |
...
到阶段 4 再增加发布时间和标签关系。封面只保存图片 URL，不在项目中实现文件上传。
...
这一章是 Mini CMS 项目任务和进度的唯一来源。从阶段 1 开始，一次只完成一个阶段。
...
- 增加错误处理中间件和只允许开发管理后台来源的 CORS 配置。
...
这是 Mini CMS 主体完成后的迁移阶段，不影响前六个阶段的完成。
