...
-> 阶段 3 前读第 10、12～13 章
-> 阶段 4 增加标签时再读第 11 章
-> 阶段 5 读第 14、14A 章，阶段 6 读第 15、15A 章
-> 13A 始终是选读
...
- 打开迁移 SQL，对照表、列和约束。
- 用 TablePro 连接本地 PostgreSQL，找到 `articles` 表。
- 在 TablePro SQL Editor 中亲手执行一轮 `INSERT / SELECT / UPDATE / DELETE`。
...
- 完善草稿、发布和撤回规则。
- 增加标题、状态和标签筛选。
- 增加发布时间字段：Prisma 代码使用 `publishedAt`，数据库列通过 `@map("published_at")` 使用 `published_at`；同时支持分页和按创建时间排序。
...
- 密码只保存哈希。
- Session Token 使用服务器生成的随机值，数据库只保存它的哈希和过期时间。
- 完成登录、退出和当前用户接口。
- 使用 HttpOnly Cookie。
- 增加认证中间件。
- 配置 CORS 和 Cookie credentials。
- 未登录不能读取草稿管理数据，也不能修改文章和标签。
- 增加 Next.js 登录页和登录状态。
...
- 数据库中没有明文密码。
- 数据库中没有可以直接当 Cookie 使用的原始 Session Token。
- 未登录请求受保护接口时返回 401。
- 登录后刷新页面仍能识别管理员。
- 退出后不能继续操作内容。
