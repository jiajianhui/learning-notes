# 15A. 登录实操：用数据库 Session 和 HttpOnly Cookie 保护写接口

## 这一章要完成什么

第 15 章先解释认证、Cookie 和基本安全。这一章只做一条可运行的登录闭环：
...
登录、退出、刷新恢复身份和写接口保护都能走通后，回到[第 10 章项目总览](./10-MiniCMS项目总览.md)完成阶段 6 验收。下一步再读第 16、16A 章，为核心 API 增加自动化测试。
