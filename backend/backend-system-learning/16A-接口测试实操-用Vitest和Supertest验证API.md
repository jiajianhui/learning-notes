# 16A. 测试实操：用 Vitest、Supertest 和独立数据库验证 API

## 这一章要完成什么

第 16 章先区分手动检查、产品流程和自动化测试。这一章把 Mini CMS 的核心 API 变成可重复执行的测试：
...
核心测试和项目 README 都完成后，回到[第 10 章项目总览](./10-MiniCMS项目总览.md)完成阶段 7 验收。Mini CMS 主体到这里已经完成；需要接入个人网站和部署时，再进入第 17 章的可选阶段 8。
