...
这里先把 `pool.query()` 理解成“向数据库发送 SQL”，第 08 章会解释 `pg` 和连接池。关键是 SQL 结构和用户数据分开传递；真实项目还要先校验 `request.params.id`，再交给执行 SQL 的函数。
...
- 把标签、作者等关系全部塞进一个 JSON 或文本字段，会失去外键、JOIN 和数据库约束；有明确关系的数据应该拆成表。
- 只在代码里查询 slug 是否重复，仍可能因为遗漏检查或同时请求而写入重复值；数据库也要设置唯一约束。
- 标题可能重复或修改，不能稳定指向一篇文章；表之间应使用不会随内容变化的 id 关联。
- 没想清楚文章、标签和删除规则就先建复杂表，之后容易反复改结构；先明确业务，再决定表和约束。
- 把客户端输入直接拼进 SQL 会产生 SQL 注入风险；使用 `$1` 等占位符和参数数组。
