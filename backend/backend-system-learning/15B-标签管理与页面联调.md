...
统一错误响应的流程见[第 15 章第 10 节](./15-数据关系JOIN和事务.md)。本章按第 1 节在标签 repository 中补充转换，错误中间件继续复用：
...
用 Apifox 分别提交重复 slug，以及更新、删除不存在的标签 id，确认提示指向标签，而不是“文章不存在”或“文章 slug 重复”。其余错误继续抛出，交给原错误中间件处理。
