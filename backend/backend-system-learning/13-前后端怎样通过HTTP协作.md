...
```text
HTTP 方法和路径 + 路径参数 / 查询参数 / 请求体字段 + 状态码 + 响应 JSON
```

例如，前端要新建文章，就必须按照后端已经提供的 `POST /api/articles` 发送数据，不能自己猜路径和字段。

API contract 为什么是数据进出后端的边界，回看[第 06 章第 4 小节](./06-API设计和参数校验.md)。本章直接使用这份约定，继续追踪浏览器和 Express 怎样传递数据。
...
本套路线会练习两种请求方式：第 14 章在 Ant Design 后台中封装 `fetch` 并完成整套文章 CRUD；第 25 章在 shadcn/ui 后台中改用 Axios 请求同一套 API。两边都会把请求细节集中到统一函数中，页面只调用文章和登录 API。
...
第 14 章会在真实页面中完成这些状态。遇到具体问题时，统一回到[第 21 章](./21-开始实践后怎样排错.md)沿请求链排查，本章不再展开故障清单。
