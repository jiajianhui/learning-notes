# 13. 多表关系和事务：增加文章标签时再学

> Mini CMS 阶段 5：先完成阶段 4 的文章管理页面。等需要给文章增加标签时，再学习这一章。
...
这一章沿用第 09 章介绍的 Prisma 7 方案，不切回手写 `pg` 查询。所有 Schema、迁移、标签接口和管理页面都继续修改真实 `mini-cms`，不再回到第 09 章 demo。
...
这一章是单表 CRUD 之后的项目扩展，不是 PostgreSQL 和 Prisma 的第一轮入门要求。标签 CRUD、文章关联、筛选分页和发布撤回都能在真实 Mini CMS 中走通后，回到 [Mini CMS 项目路线](./MiniCMS-项目路线.md)完成阶段 5 验收。下一步再读第 14、14A 章，增加管理员登录和接口保护。
