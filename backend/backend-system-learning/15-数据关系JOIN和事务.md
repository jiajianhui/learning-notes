...
先用 [15A-从单表 CRUD 到关系维护](./15A-从单表CRUD到关系维护.md)复习创建、更新和返回标签的完整流程，再到 [16-标签管理与页面联调](./16-标签管理与页面联调.md) 完成标签接口、管理页面和测试数据。最后用 [16A-从表单到数据库再回到页面](./16A-从表单到数据库再回到页面.md) 梳理前后端数据流，回到第 10 章验收阶段 5。
