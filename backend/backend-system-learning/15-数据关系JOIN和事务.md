...
这一章沿用第 09 章的 Prisma 7 方案，不切回手写 `pg` 查询。标签自己的模块和管理页面在 [16-标签管理与页面联调](./16-标签管理与页面联调.md) 中完成。
...
如果后面验证失败，先在 TablePro 中查看当前数据：关系会随着你的 PATCH 请求变化，不一定还保持第 1 节的样子。全部练习结束后，再按第 16 章第 4 节的种子脚本重建联调数据。
...
这份详情响应会用于第 16 章第 3 节的编辑回填：`article.articleTags.map((item) => item.tag.id)` 得到多选框需要的标签 id 数组。
...
`data` 仍然是文章数组，所以 `admin-web-antd` 的文章列表页现在还能正常显示第一页数据。把筛选表单和分页器接上去是页面任务，第 16 章第 3 节说明。
...
**写完关系后再查询，更新响应才能带上最终的 `articleTags`。** 前面的 `tx.article.update()` 发生在关系替换之前，默认也不返回关系，不能直接用它的结果代表最新标签。接入筛选分页后，修改过的文章还可能不再符合当前条件，因此页面按 第 16 章第 3 节重新请求当前列表，让行数据和分页总数一起更新。
...
先按 CRUD 操作看，当前文章接口会遇到哪些已知情况：

| 操作 | 文章自身的已知错误或空结果 | 本章增加的关系错误 |
|---|---|---|
| 查询列表 `getArticles()` | 没有符合条件的文章时返回空列表，总数为 0，不报“文章不存在” | 无 |
| 查询详情 `getArticleById()` | `findUnique()` 返回 `null`，Router 抛出 `AppError`，返回 404 | 无；文章没有标签时返回 `articleTags: []` |
| 创建文章 `createArticle()` | `P2002`：文章 slug 重复 | `P2025`：连接的标签不存在 |
| 更新文章 `updateArticle()` | 文章不存在时主动抛出 `AppError`，或数据库操作抛出 `P2025`；`P2002`：文章 slug 重复 | `P2003`：新关系引用的标签不存在 |
| 删除文章 `deleteArticle()` | `P2025`：文章不存在 | 删除文章会清掉对应关系，标签本身保留 |

**文章自身的错误沿用原中间件，新增的关系错误在 Repository 中转换后再抛出。** 上表只列当前实现中的已知场景，不包含所有可能的失败；数据库连接失败等未匹配的错误也要继续向上传播，由中间件兜底。

再对照具体的转换位置和响应。请求参数校验发生在 Router 中，也列在下面：
...
先用 [15A-从单表 CRUD 到关系维护](./15A-从单表CRUD到关系维护.md)复习创建、更新和返回标签的完整流程，再到 [16-标签管理与页面联调](./16-标签管理与页面联调.md) 完成标签接口、管理页面和测试数据，用 [16A-从表单到数据库再回到页面](./16A-从表单到数据库再回到页面.md) 梳理前后端数据流。随后按 [16B](./16B-用AntDesign完善后台UI.md) 完善后台 UI、[16C](./16C-用ProComponents简化管理后台.md) 完成 ProComponents 改造，再回到[第 10 章](./10-MiniCMS项目总览.md)验收阶段 5。
