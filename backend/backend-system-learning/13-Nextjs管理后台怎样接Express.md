# 13. 前后端联调：Next.js 管理后台怎样接 Express
...
进入本章时，Mini CMS 的 `server/` 已经完成阶段 3：文章 CRUD、Zod、统一错误和后端项目结构都可用。本章直接在同一个 `mini-cms` 仓库中新增 `admin-web/`，完成阶段 4。
...
CORS 是浏览器的跨来源访问规则。当前只需要允许这个管理后台地址；登录后怎样跨来源携带 Cookie，第 15 章再处理。
...
按下面的顺序推进，不要一次同时写三个页面：

```text
1. 创建 admin-web，确认 Next.js 可独立启动
2. 配置 API 地址和 Express CORS，先请求文章列表
3. 完成列表的 loading、empty、error 和 success
4. 完成新建页，用一条文章验证 POST 链路
5. 复用文章表单完成编辑页
6. 最后增加删除确认，再走一遍完整 CRUD 闭环
```
...
文章列表、新建、编辑和删除都能从页面走通后，回到[第 10 章项目总览](./10-MiniCMS项目总览.md)完成阶段 4 验收。下一步再阅读第 14 章，给文章增加标签和多表关系。`13A` 是架构对比选读，建议完成第一轮主线后再看。
