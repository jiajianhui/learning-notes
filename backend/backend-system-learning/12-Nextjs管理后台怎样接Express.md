...
进入本章时，真实 Mini CMS 的 `server/` 已经完成阶段 3：文章 CRUD、Zod、统一错误和后端项目结构都可用。本章不再创建新 demo，而是在同一个 `mini-cms` 仓库中新增 `admin-web/`，完成阶段 4。
...
CORS 是浏览器的跨来源访问规则。当前只需要允许这个管理后台地址；登录后怎样跨来源携带 Cookie，第 14 章再处理。
...
文章列表、新建、编辑和删除都能从页面走通后，回到 [Mini CMS 项目路线](./MiniCMS-项目路线.md)完成阶段 4 验收。下一步再阅读第 13 章，给文章增加标签和多表关系。`12A` 是架构对比选读，建议完成第一轮主线后再看。
