...
这八个阶段安排的是共享后端和产品能力。两个管理后台不属于其中某一个阶段，而是用同一套 API 完成的两次并列交付：
...
标签自身的 CRUD 复用阶段 2 的文章 CRUD 做法，标签管理页和文章筛选界面复用阶段 4 的页面流程；第 15 章最后一节列出这两件事分别要改什么，不再重复跟练。本阶段真正新增的是发布规则、多表关系、关联查询、筛选分页和事务。
