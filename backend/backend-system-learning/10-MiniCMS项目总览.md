...
| 前端项目 | 目录 | 主要章节 | 完成要求 |
|---|---|---|---|
| A：Ant Design | `admin-web-antd` | 第 13 章学习 `fetch`，第 14 章跟练，第 14A 章收口完整 CRUD 主线，第 14B 章深入数据和错误传递，并跟随阶段 5～6 补齐功能 | 用 `fetch` 完成登录、文章和标签管理 |
...
这是 Mini CMS 的前端起点。第 13 章先讲清前后端之间的 HTTP 边界和两个前端项目总览，第 14 章再创建真实 `admin-web-antd`。完整学完第 14 章后，第 14A 章先从页面操作一路追到数据库并返回页面，第 14B 章再放大页面、表单和 API 之间的数据与错误传递。这些章节都不会新建临时前端 demo。
