...
| 2 | 让 Mini CMS 拥有数据库能力 | PostgreSQL 真正保存文章，Prisma 可以读写数据 | 第 05～09A 章 |
| 3 | 请求和错误还不可靠，代码开始集中 | CRUD、Zod、统一错误和后端结构稳定可用 | 第 11、12 章 |
| 4 | 只有 API，没有产品操作入口 | Next.js 管理后台完成文章 CRUD 闭环 | 第 13 章 |
| 5 | 只有单表文章，没有标签和发布规则 | 多表关系、筛选分页、发布撤回和事务可用 | 第 14 章 |
| 6 | 写接口没有身份保护 | 管理员登录、Cookie 和认证中间件可用 | 第 15、15A 章 |
| 7 | 主要依赖手动检查，项目不易复现 | 核心 API 可自动验证，README 可以指导启动 | 第 16、16A 章 |
| 8（可选） | 内容只能在本地管理 | 个人网站读取已发布内容，并完成部署 | 第 17 章 |
