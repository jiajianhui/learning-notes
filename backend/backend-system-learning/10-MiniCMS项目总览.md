...
| 5 | 只有单表文章，没有标签和发布规则 | 多表关系、筛选分页、发布撤回和事务可用，后台用 ProComponents 简化实现 | 第 06、08、15、15A、16、16A、16B、16C 章 |
...
`admin-web-antd` 先用普通组件学习基础，再按第 16C 章改为 Ant Design + ProComponents。基础版通过 Git 保留，后续沿用改造后的工程，不再维护第三套后台。
...
- 按 [16C-用 ProComponents 简化管理后台](./16C-用ProComponents简化管理后台.md) 改造列表和表单，保留原有功能与操作流程，再验收本阶段。
