...
-> 用公开 API 把已发布文章提供给个人网站
-> 完成一个可以管理、发布和公开阅读文章的小型内容系统
```
...
个人网站仍保留在它自己的项目中。阶段 8 只让它读取 Mini CMS 的公开 API，不把个人网站搬进 `mini-cms` 仓库。
...
| 4 | 只有 API，没有产品操作入口 | Ant Design 后台完成文章 CRUD 闭环 | 第 13、14、14A、14B 章 |
| 5 | 只有单表文章，没有标签和发布规则 | 多表关系、筛选分页、发布撤回和事务可用，完善后台 UI 后用 ProComponents 简化实现 | 第 06、08、15、15A、16、16A、16B、16C、16D 章 |
| 6 | 写接口没有身份保护 | 管理员登录、Cookie 和认证中间件可用 | 第 17、17A 章 |
| 7 | 主要依赖手动检查，项目不易复现 | 核心 API 可自动验证，README 可以指导启动 | 第 18、18A 章 |
| 8 | 内容只能在本地管理 | 个人网站读取已发布内容，完成文章详情页和部署 | 第 06、19、19A 章 |

Ant Design 后台从阶段 4 开始，在阶段 5～6 继续补齐标签和登录功能。共享 API 稳定后，再完成 shadcn/ui 后台。两个后台使用同一套 API，最终完成相同的核心管理功能：

| 前端项目 | 目录 | 主要章节 | 完成要求 |
|---|---|---|---|
| A：Ant Design | `admin-web-antd` | 第 13 章学习 `fetch`，第 14 章跟练，第 14A 章收口完整 CRUD、数据返回和错误传播，第 14B 章复习语言与状态机制，并跟随阶段 5～6 补齐功能 | 用 `fetch` 完成登录、文章和标签管理 |
| B：shadcn/ui | `admin-web-shadcn` | 第 24～28 章 | 用 Axios 完成同等核心链路，并掌握 TanStack Table、React Hook Form |

项目 A 先实现，项目 B 后实现。顺序用于控制学习难度，最终验收时两者地位相同。

`admin-web-antd` 先用普通组件学习基础，按第 16C 章对照 Ant Design Pro 演示完善 UI，再按第 16D 章改为 Ant Design + ProComponents，保持相同的视觉目标与业务行为。普通组件版通过 Git 保留，后续沿用改造后的工程，不再维护第三套后台。
...
到阶段 5 再增加发布时间和标签关系；阶段 8 接入个人网站时，再按详情页需要决定是否增加可为空的 `coverImageUrl`。封面只保存图片 URL，不在项目中实现文件上传。
...
- 阶段 2～7 中的 `content` 继续是数据库 `String`，后端只校验和保存字符串；完成后端主线后，阶段 8 再根据个人网站现有的 Markdown / MDX 能力决定正文解析方案。
- 不做评论、点赞、收藏、全文搜索引擎和云存储。
- 不为了架构形式机械拆很多层。
- 发布和撤回先作为文章状态更新，基本 CRUD 稳定后再决定是否拆成独立接口。
- 不在阶段 8 之前替换现有个人网站的数据来源。
...
这是 Mini CMS 的前端起点。第 13 章先讲清前后端之间的 HTTP 边界和两个前端项目总览，第 14 章再创建真实 `admin-web-antd`。完整学完第 14 章后，第 14A 章从页面操作一路追到数据库，并沿成功数据或错误返回页面；第 14B 章再集中复习泛型、Promise、类型和 React 状态。这些章节都不会新建临时前端 demo。

本阶段按下面的顺序阅读：

- [13-前后端怎样通过 HTTP 协作](./13-前后端怎样通过HTTP协作.md)
- [14-Ant Design 管理后台跟练](./14-Ant-Design管理后台跟练.md)
- [14A-从页面操作到数据库：一条线看懂管理后台 CRUD](./14A-从页面操作到数据库-一条线看懂管理后台CRUD.md)
- [14B-管理后台里的 TypeScript、Promise 和 React 状态](./14B-管理后台里的TypeScript-Promise和React状态.md)
...
- 完成第 15 章后，用 [15A-从单表 CRUD 到关系维护](./15A-从单表CRUD到关系维护.md)复习文章、标签和关系的完整流程。
- 接着按 [16-标签管理与页面联调](./16-标签管理与页面联调.md) 完成标签接口、管理页面和测试数据。
- 用 [16A-从表单到数据库再回到页面](./16A-从表单到数据库再回到页面.md) 复习前后端数据流。
- 再读 [16B-页面变大后怎样拆组件](./16B-页面变大后怎样拆组件.md)，理解组件封装和状态归属，不要求逐项实施拆分。
- 按 [16C-用 Ant Design 完善后台 UI](./16C-用AntDesign完善后台UI.md) 理解布局与样式的分工，自主还原目标页面并留下记录。
- 按 [16D-用 ProComponents 简化管理后台](./16D-用ProComponents简化管理后台.md) 改造列表、表单和布局，保持相同的 UI 与操作流程，再验收本阶段。

第 15、16 章共同完成本阶段：第 15 章建立多表关系、发布规则、筛选分页和事务；第 16 章完成标签接口、管理页面、表单关联和分页联调。标签 CRUD 复用前面的做法独立练习，首次接入的表单数据和分页请求提供局部代码。15A 巩固关系维护，16A 巩固前后端数据流。
...
- 普通 Ant Design 版与 ProComponents 版对齐同一个 UI 目标：顶栏与导航固定，内容不被遮挡，列表、抽屉及加载和错误提示风格协调；保留普通组件版的截图与关键实现记录。
- ProComponents 改造后，新建、编辑、删除、错误提示和重新获取列表的行为仍可验证，没有保留两套重复的列表请求。
...
- [17-登录、Cookie 和安全](./17-登录Cookie和基本安全.md)
- [17A-管理员登录实操](./17A-管理员登录实操-用Session和Cookie保护写接口.md)
...
- [18-后端测试](./18-后端测试怎么分层.md)
- [18A-接口测试实操](./18A-接口测试实操-用Vitest和Supertest验证API.md)
...
入口：[第 24 章](./24-shadcn-ui为什么不是传统组件库.md)。
...
- [06-API 设计和校验](./06-API设计和参数校验.md)
- [19-公开内容和生产运行](./19-从开发环境到线上运行.md)
- [19A-用 1Panel 部署 Mini CMS](./19A-用1Panel和Docker-Compose部署MiniCMS.md)

公开文章路由复用前面已经掌握的 GET 接口、筛选和错误处理。第 19 章先连通“发布文章 -> 公开 API -> 个人网站详情页”，再建立生产运行地图；第 19A 章用 1Panel、Docker Compose 和 OpenResty 完成自托管部署。本项目不使用宝塔。

后端主线完成前，`content` 始终保持 `String`，不在阶段 5～7 提前引入正文解析器。到阶段 8 再处理正文结构和页面排版：

```text
任一管理后台输入 content 字符串
-> Express 把字符串保存到 PostgreSQL
-> 公开 API 返回已发布文章的 content
-> 个人网站按阶段 8 选定的 Markdown 或 MDX 方案解析正文
-> 文章详情组件和 CSS 决定字体、宽度、间距、图片和代码块样式
```

Markdown 用文本语法表示标题、段落、列表、引用、代码块和图片；MDX 在 Markdown 之上还允许使用受控的组件。两者都不决定最终视觉，并且在数据库中都仍然可以作为 `String` 保存。标题、slug、摘要、状态、发布时间和标签继续使用独立的数据库字段，不和正文字符串混在一起。

最终文章详情页的布局和正文排版参考：

- [HEY World 的 DHH 文章](https://world.hey.com/dhh/linux-as-the-new-developer-default-at-37signals-ef0823b7)：参考窄栏、留白、简洁信息和专注正文的整体结构。
- [Vercel Blog 的 `design.md` 文章](https://vercel.com/blog/how-our-agents-build-on-brand-pages-with-design-md)：参考标题层级、长文节奏、大图、链接和代码内容的排版。

这两个链接只用于参考文章详情页布局和正文排版，不复制对方的站点导航、品牌标识或内容存储方案。

完成：

- 提供只返回已发布文章的公开 API。
- 让个人网站读取文章列表和详情。
- 如果详情页需要独立封面，再为文章增加可为空的 `coverImageUrl`；只保存图片 URL，不实现文件上传。
- 检查个人网站现有的 Markdown / MDX 实现，再选定 `content` 的解析方案，不因此修改数据库的 `String` 类型。
- 把 `content` 渲染成语义清晰的正文，完成文章详情页布局，并接入封面图片地址。
...
- 用 1Panel 管理 Docker Compose、OpenResty 反向代理、HTTPS 证书和备份任务。
- 确认数据库备份和恢复方式，再部署 Express API、PostgreSQL 和计划上线的一套或两套管理后台。

验收：

- 未登录用户通过公开内容 API 只能读取已发布文章；访问草稿、管理数据或写接口时仍然被拒绝。
- 个人网站能展示来自 Mini CMS 的文章列表和详情，选定的正文格式能稳定渲染标题、段落、列表、引用、代码块、链接和图片。
- 文章详情页布局和正文排版能看出 HEY World 的简洁阅读结构和 Vercel Blog 的内容层级参考，但不要求像素级复制。
- 新环境可以根据 README 和环境变量完成构建、迁移和启动。
- 1Panel 只管理 Compose、网站、证书和备份，项目运行关系仍然保存在仓库配置中。
- 线上健康检查可用，失败时可以通过日志定位，日志中没有敏感信息。
- 数据库迁移记录与代码版本对应，并已经确认备份和恢复方式。

这是 Mini CMS 的最后一个必做阶段。个人网站能读取正式内容、文章详情页达到排版目标，并且真实部署链路验证通过后，才算完成八个阶段。
...
- 个人网站能通过正式 API 展示文章列表和详情，草稿不会出现在公开页面。
- 文章详情页完成已确认的布局和正文排版，而不只是把 `content` 字符串直接显示出来。
