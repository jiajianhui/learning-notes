...
| 5 | 只有单表文章，没有标签和发布规则 | 多表关系、筛选分页、发布撤回和事务可用 | 第 06、08、15、15A、15B、15C 章 |
| 6 | 写接口没有身份保护 | 管理员登录、Cookie 和认证中间件可用 | 第 16、16A 章 |
| 7 | 主要依赖手动检查，项目不易复现 | 核心 API 可自动验证，README 可以指导启动 | 第 17、17A 章 |
| 8 | 内容只能在本地管理 | 个人网站读取已发布内容，完成文章详情页和部署 | 第 06、18、18A 章 |
...
- 接着按 [15B-标签管理与页面联调](./15B-标签管理与页面联调.md) 完成标签接口、管理页面和测试数据。
- 最后用 [15C-从表单到数据库再回到页面](./15C-从表单到数据库再回到页面.md) 复习前后端数据流，再验收本阶段。

标签自身的 CRUD 复用阶段 2 的文章 CRUD 做法，标签管理页和文章筛选界面复用阶段 4 的页面流程；第 15B 章列出这两件事分别要改什么，不再重复跟练。本阶段真正新增的是发布规则、多表关系、关联查询、筛选分页和事务。
...
- [06-API 设计和校验](./06-API设计和参数校验.md)
- [18-公开内容和生产运行](./18-从开发环境到线上运行.md)
- [18A-用 1Panel 部署 Mini CMS](./18A-用1Panel和Docker-Compose部署MiniCMS.md)

公开文章路由复用前面已经掌握的 GET 接口、筛选和错误处理。第 18 章先连通“发布文章 -> 公开 API -> 个人网站详情页”，再建立生产运行地图；第 18A 章用 1Panel、Docker Compose 和 OpenResty 完成自托管部署。本项目不使用宝塔。
