...
| 4 | 只有 API，没有产品操作入口 | Ant Design 后台完成文章 CRUD 闭环 | 第 13、14、14A、14B、14C 章 |
...
| 8（可选） | 内容只能在本地管理 | 个人网站读取已发布内容，并完成部署 | 第 06、18、18A 章 |

八个阶段组织共享后端和产品能力，两个前端项目则是并列交付：

| 前端项目 | 目录 | 主要章节 | 完成要求 |
|---|---|---|---|
| A：Ant Design | `admin-web-antd` | 第 13 章学习 `fetch`，第 14 章跟练，第 14A 章收口完整 CRUD 主线，第 14B 章深入数据和错误传递，第 14C 章复习语言与状态机制，并跟随阶段 5～6 补齐功能 | 用 `fetch` 完成登录、文章和标签管理 |
...
这是 Mini CMS 的前端起点。第 13 章先讲清前后端之间的 HTTP 边界和两个前端项目总览，第 14 章再创建真实 `admin-web-antd`。完整学完第 14 章后，第 14A 章先从页面操作一路追到数据库并返回页面，第 14B 章再放大页面、表单和 API 之间的数据与错误传递，第 14C 章最后集中复习泛型、Promise、类型和 React 状态。这些章节都不会新建临时前端 demo。
...
- [14B-数据和错误怎样在页面与表单之间传递](./14B-数据和错误怎样在页面与表单之间传递.md)
- [14C-管理后台里的 TypeScript、Promise 和 React 状态](./14C-管理后台里的TypeScript-Promise和React状态.md)
...
公开文章路由复用前面已经掌握的 GET 接口、筛选和错误处理；第 18 章先建立生产运行地图，第 18A 章再用 1Panel、Docker Compose 和 OpenResty 完成自托管部署。本项目不使用宝塔。
