...
| 5 | 只有单表文章，没有标签和发布规则 | 多表关系、筛选分页、发布撤回和事务可用 | 第 06、08、15、15A 章 |
...
标签自身的 CRUD 复用阶段 2 的文章 CRUD 做法，标签管理页和文章筛选界面复用阶段 4 的页面流程；第 15 章第 10 节列出这两件事分别要改什么，不再重复跟练。本阶段真正新增的是发布规则、多表关系、关联查询、筛选分页和事务。
