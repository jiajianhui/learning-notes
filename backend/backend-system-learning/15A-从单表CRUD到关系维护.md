...
更新文章的第一步 `tx.article.update()` 发生在关系替换之前，而且默认不会返回标签关系。直接把它的结果交给页面，页面拿不到最终的标签。
...
- 不清楚错误提示和页面怎样衔接：回看第 15 章第 10 节。

能说清“创建文章时新增哪些行、更新标签时替换哪些行、失败时撤销哪些修改、页面需要读回什么”，就回到[第 10 章](./10-MiniCMS项目总览.md)验收阶段 5，再继续第 16、16A 章的管理员登录。
