...
能说清“创建文章时新增哪些行、更新标签时替换哪些行、失败时撤销哪些修改、页面需要读回什么”，就继续阅读 [16-标签管理与页面联调](./16-标签管理与页面联调.md)，完成标签管理、页面联调和测试数据，再用 [16A-从表单到数据库再回到页面](./16A-从表单到数据库再回到页面.md) 复习数据流，回到第 10 章验收阶段 5。
