...
- 不清楚页面怎样接入：阅读 [16-标签管理与页面联调](./16-标签管理与页面联调.md) 第 3 节。

能说清“创建文章时新增哪些行、更新标签时替换哪些行、失败时撤销哪些修改、页面需要读回什么”，就继续阅读 [16-标签管理与页面联调](./16-标签管理与页面联调.md)，完成标签管理、页面联调和测试数据，再用 [16A-从表单到数据库再回到页面](./16A-从表单到数据库再回到页面.md) 复习数据流。随后按 [16B](./16B-用AntDesign完善后台UI.md) 完善后台 UI、[16C](./16C-用ProComponents简化管理后台.md) 完成 ProComponents 改造，再回到[第 10 章](./10-MiniCMS项目总览.md)验收阶段 5。
