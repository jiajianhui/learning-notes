# 15. 登录和安全：服务器怎样确认管理员身份

> Mini CMS 阶段 6：先完成标签、发布规则和管理页面，再用本章理解登录闭环；具体代码放到第 15A 章实践。
...
如果已按第 13 章配置 Express 允许管理后台的来源，登录改用 Cookie 后，还需要让后端 CORS 允许凭证：
...
下一步不要停在概念层。进入 [15A-管理员登录实操](./15A-管理员登录实操-用Session和Cookie保护写接口.md)，亲手完成密码哈希、数据库 Session、HttpOnly Cookie 和写接口保护。
