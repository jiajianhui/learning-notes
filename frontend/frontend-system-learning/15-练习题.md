...
这些练习不用一口气做完，可以按阅读进度分批做：

| 读到哪里 | 优先做什么 |
|---|---|
| 读完 02 | 做 01 原生版 |
| 读完 07 | 做 Vite + Vanilla TS / Vue / React 三个 demo 版 |
| 读完 09 / 10 | 做项目类型判断题 |
| 读完 11 / 12 | 做技术分层和学习路线题 |
...
```text
HTML/CSS/JS -> TypeScript -> React/Vue 最小语法 -> Vite -> 路由 -> 状态管理
```
...
```text
HTML/CSS/JS -> React/Vue 最小语法 -> Vite -> 理解 CSR/SSR/SSG -> Next.js 或 Nuxt.js
```
...
同一个计数器功能，分别用下面三种方式实现。请说明它们分别体现了哪一层能力。
...
原生 JavaScript 体现的是第一层基础能力：

```text
JavaScript 通过 DOM API 找到元素
监听点击事件
手动修改页面文本
```

Vue 和 React 体现的是第三层 UI 框架能力：
...
Vue 和 React 的共同点是“数据驱动视图”，差异是表达方式：

```text
Vue：模板 + ref
React：JSX/TSX + useState
```

这个练习的重点不是背写法，而是看懂：同一个功能在不同层里的表达方式不同。
...
| 现象 | 谁在工作 |
|---|---|
| 执行 `npm run dev` 后项目启动 | ？ |
| 浏览器打开 `localhost:5173` | ？ |
| `.vue` 文件可以被项目识别 | ？ |
| 点击按钮后 React 页面更新 | ？ |
| URL 从 `/` 变成 `/todos` 但没有整页刷新 | ？ |
| 两个页面共享同一个 count | ？ |
...
| 现象 | 谁在工作 |
|---|---|
| 执行 `npm run dev` 后项目启动 | npm 读取 `package.json` 并执行 `scripts.dev` |
| 浏览器打开 `localhost:5173` | Vite 开发服务器 |
| `.vue` 文件可以被项目识别 | Vite 的 Vue 插件和 Vue 编译能力 |
| 点击按钮后 React 页面更新 | React 的状态和渲染机制 |
| URL 从 `/` 变成 `/todos` 但没有整页刷新 | 前端路由 |
| 两个页面共享同一个 count | store / 状态管理 |

这个练习要训练的是归因能力：不要把所有现象都归到 React/Vue 身上。
