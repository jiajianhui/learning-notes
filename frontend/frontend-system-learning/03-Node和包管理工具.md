...
### 1. 浏览器和 Node.js 分工不同：一个给用户，一个给开发工具
...
### 3. 早期为什么不需要 Node：页面简单时直接打开 HTML
...
### 5. React / Vue 是否强绑定 Node：小 demo 不一定，现代项目通常需要
...
这一章站在 npm 包使用者的角度。读完 TypeScript、Vite 和项目阅读章节后，可以用 [11A-从使用者到作者：制作、验证并发布一个 npm 包](./11A-从使用npm包到制作并发布npm包.md) 完成作者侧的构建、打包和本地安装验证。
